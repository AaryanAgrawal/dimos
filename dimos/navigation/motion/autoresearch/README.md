# autoresearch: the motion-planner benchmark

Standalone referee for the dimos local planner: ground-truth 2D worlds, an
SE(2) gold oracle, a judge that scores candidate plans against exact truth,
and the Rust candidate crate (`rust/`, pyo3 module `dimos_motion2_target`).
...
(`export/`), which packages the benchmark for evo autoresearch loops.
