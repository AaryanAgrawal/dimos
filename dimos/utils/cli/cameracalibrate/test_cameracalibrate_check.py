...
def test_check_calibration_verdict_tracks_intrinsics(fx_scale: float, verdict: str) -> None:
    """Deployed K/D == truth -> OK at low RMS + small drift; fx off 8% -> DEGRADED, high RMS, drift."""
    views = _synthetic_board_corners(count=14)
    K = _K_TRUE.copy()
    K[0, 0] *= fx_scale
    result = check_calibration(views, **_camera_info_dict(K, _D_TRUE))  # type: ignore[arg-type]
...
@pytest.mark.parametrize(("fx_scale", "verdict"), [(1.0, "OK"), (1.08, "DEGRADED")])
def test_cli_check_folder_verdict_and_json(tmp_path: Path, fx_scale: float, verdict: str) -> None:
    """`--check --source folder` end-to-end: matched YAML -> OK, fx off 8% -> DEGRADED, JSON written."""
    ...
    result = CliRunner().invoke(
        app,
        [
            "--check", "--source", "folder", "--images", str(images),
            "--camera-info", str(deployed), "--cols", "9", "--rows", "6",
            "--square-size-m", "0.025", "--out", str(out_json), "--no-display",
        ],
    )  # fmt: skip
